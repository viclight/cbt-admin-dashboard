...
import { getQuestionsBySubject, saveTestSession } from '@/lib/database';
...
const Test = () => {
  ...
  useEffect(() => {
    ...
    // Load questions for the subject from backend
    if (subject) {
      (async () => {
        try {
          const testQuestions = await getQuestionsBySubject(subject, 20);
          setQuestions(Array.isArray(testQuestions) ? testQuestions : []);
        } catch (err) {
          setQuestions([]);
        }
      })();
